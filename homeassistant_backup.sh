...
# from https://community.home-assistant.io/t/backup-configuration-to-a-usb-drive/196964

BACKUP_FOLDER=/var/backups/homeassistant/
BACKUP_FILE=${BACKUP_FOLDER}hass-config_$(date +"%Y%m%d_%H%M%S").zip
HOMEASSISTANT_LOCATION=/etc/homeassistant
INCLUDE_DB=true
DAYSTOKEEP=7 # Set to 0 to keep it forever.
...
if [ -d "${BACKUP_FOLDER}" ]; then
        if [ ! -d "${HOMEASSISTANT_LOCATION}" ]; then
                log e "Homeassistant folder not found, is it correct?" 1
        fi
        pushd ${HOMEASSISTANT_LOCATION} >/dev/null
        ...
        if [ "${DAYSTOKEEP}" = 0 ] ; then
                ...
                OLDFILES=$(find ${BACKUP_FOLDER}/hass-config* -mindepth 1 -mtime +${DAYSTOKEEP} -delete -print)
                if [ ! -z "${OLDFILES}" ] ; then
        ...
        log e "Backup folder not found, is your location mounted?" 1
fi
